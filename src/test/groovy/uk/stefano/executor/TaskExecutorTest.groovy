...
class TaskExecutorTest extends Specification {
    ...
    def 'the executor should only schedule each task once'(){
        ...
        given:
        def theList = []
        executor.buildTaskFromDependencyInformation(4, [2,3],new StackingRunnable(4,0,theList));
        executor.buildTaskFromDependencyInformation(2, [1],new StackingRunnable(2,100,theList));
        executor.buildTaskFromDependencyInformation(3, [1],new StackingRunnable(3, 0, theList));
        executor.buildTaskFromDependencyInformation(1, [],new StackingRunnable(1, 0, theList));
        when:
    ...
    def 'the executor should correctly handle unreachable nodes'(){

        given:
            def theList = []
            executor.buildTaskFromDependencyInformation(5, [4],new StackingRunnable(4,0,theList));
            executor.buildTaskFromDependencyInformation(2, [1],new StackingRunnable(2,100,theList));
            executor.buildTaskFromDependencyInformation(3, [1],new StackingRunnable(3, 0, theList));
            executor.buildTaskFromDependencyInformation(1, [],new StackingRunnable(1, 0, theList));
        when:
    ...
    @Ignore
    def 'the executor should correctly detect a node with an unsatisfiable dependency list'(){
        given:
            def theList = []
            executor.buildTaskFromDependencyInformation(5, [3,4],new StackingRunnable(4,0,theList));
            executor.buildTaskFromDependencyInformation(2, [1],new StackingRunnable(2,100,theList));
            executor.buildTaskFromDependencyInformation(3, [1],new StackingRunnable(3, 0, theList));
            executor.buildTaskFromDependencyInformation(1, [],new StackingRunnable(1, 0, theList));
        when:
